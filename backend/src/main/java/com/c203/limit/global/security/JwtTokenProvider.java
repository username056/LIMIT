...
public class JwtTokenProvider {
    ...
    public JwtTokenProvider(ObjectMapper objectMapper,
            @Value("${limit.security.jwt.secret:local-only-change-this-jwt-secret-32bytes}") String secret,
    ...
    public TokenClaims parse(String token, String expectedType) {
        try {
            ...
            if (!expectedType.equals(payload.path("typ").asText())) throw new BusinessException(ErrorCode.INVALID_TOKEN);
            if (Instant.ofEpochSecond(payload.path("exp").asLong()).isBefore(Instant.now())) {
                throw new BusinessException(ErrorCode.EXPIRED_TOKEN);
            }
            Set<String> roles = objectMapper.convertValue(payload.path("roles"),
                    objectMapper.getTypeFactory().constructCollectionType(Set.class, String.class));
            return new TokenClaims(payload.path("jti").asText(), payload.path("sid").asLong(),
                    payload.path("act").asText(), roles);
    ...
    private IssuedToken issue(Long id, String accountType, Set<String> roles, String type, Duration ttl) {
        try {
            Instant expiresAt = Instant.now().plus(ttl);
            String tokenId = UUID.randomUUID().toString();
            String header = ENCODER.encodeToString("{\"alg\":\"HS256\",\"typ\":\"JWT\"}".getBytes(StandardCharsets.UTF_8));
            String json = objectMapper.writeValueAsString(new Payload(tokenId, id, accountType, roles, type, expiresAt.getEpochSecond()));
    ...
    private record Payload(String jti, Long sid, String act, Set<String> roles, String typ, long exp) {}
    public record IssuedToken(String value, String tokenId, Instant expiresAt) {}
    public record TokenClaims(String tokenId, Long subjectId, String accountType, Set<String> roles) {}
}
