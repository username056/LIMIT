 ...
 * 구매자에게 보여 주는 판매자 프로필.
 *
 * <p>{@link SellerProfileResponse}(본인 조회)와 일부러 분리했다. Seller 엔티티에는 정산 은행명·예금주·
 * 계좌 끝 4자리와 사업자 상호가 들어 있는데, 그건 판매자 본인만 볼 값이다. 이 DTO에는 구매자가
 * 거래 상대를 가늠하는 데 필요한 것만 담는다.
 */
...
public record PublicSellerProfileResponse(
        ...
        @Schema(
                        description = "개인/사업자 구분. 판매자 등록 행이 없는 회원이면 null.",
                        example = "INDIVIDUAL")
                String sellerType,
        @Schema(description = "판매자 등록 시각. 판매자 등록 행이 없으면 null.")
