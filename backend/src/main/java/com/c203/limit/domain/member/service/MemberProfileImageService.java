 ...
 * 회원 프로필 사진 업로드.
 *
 * <p>상품 이미지(ListingImageUploadService)와 달리 임시 키 → 최종 키로 옮기는 업로드 세션을 두지 않는다.
 * 상품 사진은 열 장까지 순서·대표 여부가 얽히고 판매글 소유 검증이 필요해 그 장치가 필요했다. 프로필은
 * 한 장뿐이고 주인이 곧 요청자라, 최종 키를 서버가 정해 그 키로만 쓸 수 있는 URL을 내주면 충분하다.
 *
 * <p>키를 서버가 만들기 때문에 회원이 남의 자리에 덮어쓸 수 없다. 완료 통보 때 S3에 실제로 올라왔는지
 * head로 확인한다 — 확인하지 않으면 올리지 않고 완료만 불러 깨진 사진이 박힌다.
 */
@Service
