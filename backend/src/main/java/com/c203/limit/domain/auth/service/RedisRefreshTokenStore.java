...
public class RedisRefreshTokenStore implements RefreshTokenStore {
    ...
    public void save(String tokenId, Long subjectId, Duration ttl) {
        redis.opsForValue().set(TOKEN_PREFIX + tokenId, subjectId.toString(), ttl);
        String subjectKey = SUBJECT_PREFIX + subjectId;
        redis.opsForSet().add(subjectKey, tokenId);
        redis.expire(subjectKey, ttl);
    }
    public boolean isValid(String tokenId, Long subjectId) {
        return subjectId.toString().equals(redis.opsForValue().get(TOKEN_PREFIX + tokenId));
    }
    public void revoke(String tokenId) { redis.delete(TOKEN_PREFIX + tokenId); }
    public void revokeAll(Long subjectId) {
        String subjectKey = SUBJECT_PREFIX + subjectId;
        Set<String> tokenIds = redis.opsForSet().members(subjectKey);
        if (tokenIds != null && !tokenIds.isEmpty()) redis.delete(tokenIds.stream().map(id -> TOKEN_PREFIX + id).toList());
