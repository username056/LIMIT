...
@ConditionalOnProperty(
        name = "limit.security.refresh-store",
        havingValue = "memory",
        matchIfMissing = true)
public class InMemoryRefreshTokenStore implements RefreshTokenStore {
    ...
    public void save(String tokenId, Long subjectId, String accountType, Duration ttl) {
        entries.put(tokenId, new Entry(subjectId, accountType, Instant.now().plus(ttl)));
    }

    public boolean isValid(String tokenId, Long subjectId, String accountType) {
        Entry entry = entries.get(tokenId);
        return entry != null
                && entry.subjectId().equals(subjectId)
                && entry.accountType().equals(accountType)
                ...
                .removeIf(
                        e ->
                                e.getValue().subjectId().equals(subjectId)
                                        && e.getValue().accountType().equals(accountType));
    }

    private record Entry(Long subjectId, String accountType, Instant expiresAt) {}
}
