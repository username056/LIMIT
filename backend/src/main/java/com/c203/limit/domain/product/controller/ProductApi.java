...
public interface ProductApi {
    ...
    @Operation(operationId = "product1", summary = "상품 등록", description = "요청\n권한: SELLER / multipart/form-data - request(JSON): {\"brandId\":12,\"categoryId\":3,\"name\":\"Air Jordan 1 Retro High OG\",\"description\":\"정품 미개봉 제품입니다.\",\"price\":259000,\"quantity\":50} + images(파일들) + authenticityProofs(파일들)", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses({
        @ApiResponse(responseCode = "201", description = "상품 등록 성공", content = @Content(schema = @Schema(ref = "#/components/schemas/ProductDetailResponse"))),
        @ApiResponse(responseCode = "401", description = "UNAUTHORIZED"),
        @ApiResponse(responseCode = "400", description = "VALIDATION_FAILED"),
        @ApiResponse(responseCode = "422", description = "INVALID_FILE_TYPE / MALICIOUS_FILE_DETECTED")
    })
    ...
    @Operation(operationId = "product8", summary = "상품 숨김·판매중지 / 상품 조치 해제", description = "요청\n권한: SELLER(본인) | ADMIN(전체) / Path: productId(long) / Body: {\"actionType\":\"SUSPEND\",\"reason\":\"정품 증빙 자료 진위 확인 불가로 판매 중지 처리합니다.\"} / actionType 구분 없이 소유 여부로만 판단\n\n권한: SELLER(본인) | ADMIN(전체) / Path: productId(long) / Body: {\"actionType\":\"RELEASE\",\"reason\":\"소명 자료 제출 완료로 조치를 해제합니다.\"} / 직전 조치가 누구 것이든 소유 셀러·관리자면 해제 가능(정책 재검토 필요)", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses({
        @ApiResponse(responseCode = "201", description = "상품 숨김·판매중지 / 상품 조치 해제 성공", content = @Content(schema = @Schema(ref = "#/components/schemas/ProductActionResponse"))),
        @ApiResponse(responseCode = "401", description = "UNAUTHORIZED"),
        @ApiResponse(responseCode = "400", description = "VALIDATION_FAILED"),
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "상품 목록 조회 성공", content = @Content(array = @ArraySchema(schema = @Schema(ref = "#/components/schemas/ProductSummaryResponse"))))
    })
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "이미지 조회 성공", content = @Content(array = @ArraySchema(schema = @Schema(ref = "#/components/schemas/ProductImageResponse")))),
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "상품 상세 조회 성공", content = @Content(schema = @Schema(ref = "#/components/schemas/ProductDetailResponse"))),
        @ApiResponse(responseCode = "404", description = "PRODUCT_NOT_FOUND")
    })
    ...
    @Operation(operationId = "product6", summary = "셀러 상품 목록", description = "요청\n권한: SELLER(본인) / Query: name, brandId, categoryId, saleStatus, hasReport, page, size, sort / seller_id는 토큰 강제, HIDDEN·SUSPENDED 포함 전체 조회 가능", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "셀러 상품 목록 성공", content = @Content(array = @ArraySchema(schema = @Schema(ref = "#/components/schemas/ConsoleProductSummaryResponse")))),
        @ApiResponse(responseCode = "401", description = "UNAUTHORIZED")
    })
    ...
    @Operation(operationId = "product7", summary = "관리자 상품 목록", description = "요청\n권한: ADMIN / Query: name, sellerId, brandId, categoryId, saleStatus, hasReport, page, size, sort / 전체 판매자 상품 대상", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "관리자 상품 목록 성공", content = @Content(array = @ArraySchema(schema = @Schema(ref = "#/components/schemas/ConsoleProductSummaryResponse")))),
        @ApiResponse(responseCode = "401", description = "UNAUTHORIZED"),
        @ApiResponse(responseCode = "403", description = "FORBIDDEN")
    })
    ...
    @Operation(operationId = "product10", summary = "상품 조치 이력 조회", description = "요청\n권한: SELLER(본인) | ADMIN(전체) / Path: productId(long) / Query: page, size (기본 최신순 정렬)", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "상품 조치 이력 조회 성공", content = @Content(array = @ArraySchema(schema = @Schema(ref = "#/components/schemas/ProductActionResponse")))),
        @ApiResponse(responseCode = "401", description = "UNAUTHORIZED"),
        @ApiResponse(responseCode = "403", description = "FORBIDDEN")
    })
    ...
    @Operation(operationId = "product2", summary = "상품 수정", description = "요청\n권한: SELLER(본인) / Path: productId(long) / Body(부분 수정): {\"price\":249000,\"description\":\"설명 수정본입니다.\",\"quantity\":40}", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "상품 수정 성공", content = @Content(schema = @Schema(ref = "#/components/schemas/ProductDetailResponse"))),
        @ApiResponse(responseCode = "401", description = "UNAUTHORIZED"),
        @ApiResponse(responseCode = "403", description = "FORBIDDEN"),
        @ApiResponse(responseCode = "404", description = "PRODUCT_NOT_FOUND"),
        @ApiResponse(responseCode = "409", description = "PRODUCT_EDIT_NOT_ALLOWED")
    })
