...
public interface ProductApi {
    ...
    @Operation(operationId = "product01", summary = "상품 등록", description = "ACTIVE 판매자만 상품 초안을 생성할 수 있습니다. 모든 모델은 최신 게시 체크리스트를 기본으로 고정하며, 판매자가 확인한 모델별 기능만 추가해 매물 스냅샷으로 저장합니다.", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses({
    ...
    ResponseEntity<ApiResponse<List<ProductSummaryResponse>>> getProducts(
            ...
            @RequestParam(required = false) Integer minVerifiedCount,
            @RequestParam(required = false) Integer maxVerifiedCount,
