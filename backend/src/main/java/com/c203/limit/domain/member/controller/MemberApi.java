...
public interface MemberApi {
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "내 회원 정보 조회 성공", content = @Content(schema = @Schema(ref = "#/components/schemas/MemberProfileResponse"))),
    ...
    @Operation(operationId = "member02", summary = "내 회원 정보 수정", description = "요청\n권한: MEMBER\n검증: 변경할 필드만 전달, 닉네임 중복 확인, 이메일·상태·역할 직접 수정 불가", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "내 회원 정보 수정 성공", content = @Content(schema = @Schema(ref = "#/components/schemas/UpdateMemberResponse"))),
