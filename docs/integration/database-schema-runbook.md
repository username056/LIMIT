...
### V20260802 판매자 migration 장애 대응 기록 (종료)

`V20260802` 운영 장애에 사용한 읽기 전용 진단과 승인 복구 Job은 복구 완료 후 제거했다. 당시
절차와 스크립트는 Git 이력의 `e9b6130`(진단), `7f72212`(복구·재배포)에서 감사 목적으로 확인할
수 있지만, 현재 운영 절차로 다시 실행해서는 안 된다.

`backend/src/main/resources/db/maintenance/V20260802__prepare_failed_seller_migration.sql`은 당시
스키마 보정의 재현과 migration 통합 검증을 위해 보존한다. 이미 적용된 `V20260802`를 다시
실행하거나 실패 이력을 임의 삭제·`repair`하는 용도가 아니다.

같은 증상이 다시 나타나더라도 과거 Job을 복원해 실행하지 않는다. 위의 일반 실패·복구 절차에 따라
현재 스키마와 이력을 새로 진단하고, 운영 승인을 거쳐 백업 복원 또는 더 높은 버전의 후속 migration을
선택한다. 진단 로그에는 회원 ID, 이메일, 이름, 연락처, 계좌 데이터 등 개인정보를 출력하지 않는다.
