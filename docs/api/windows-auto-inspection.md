...
포인터의 기존 이동·클릭·스크롤 판정 계약은 유지한다. 진단 프로그램 결과는 `자동 점검 완료`,
웹 결과는 `웹 점검 완료`로 구분해 표시한다.

상품 임시 저장 진행 조회 응답은 웹 직접 점검 결과인 `deviceResults`와 별도로
`automaticDeviceResults`를 제공한다. `automaticDeviceResults`는 체크리스트 항목 매핑 여부와
관계없이 해당 상품에 제출된 검사 유형별 최신 Scanner 결과다. 따라서 공통 화면에서 가상으로
추가한 키보드·포인터 항목도 Scanner 최종 제출 직후와 화면 재접속 후 모두 자동 점검 상태를
복원할 수 있다.

기존 API enum과 이전에 저장된 나머지 testType 결과는 하위 호환을 위해 유지하지만 신규 Scanner와
웹 화면에서는 스피커·디스플레이·충전·카메라·마이크 검사를 실행하거나 제출하지 않는다.
