...
OS_VERSION은 에디션("Windows 11 Enterprise")·버전("25H2")·"장치 사양" 표의 시스템 종류("64비트 운영
체제, x64 기반 프로세서") 세 조각을 이 순서로 이어 붙인 값이다 — 세 조각 중 일부만 인식돼도 인식된 조각만
공백 하나로 이어 붙이고 빈 자리나 이중 공백을 남기지 않는다. 체크리스트 안내는 "설정 > 시스템 > 정보" 화면
대신 msinfo32(시스템 정보) 캡처도 허용하는데, 이 화면은 라벨이 달라서(에디션→"OS 이름", 버전 값도 "25H2"가
아니라 "10.0.26200 빌드 26200" 같은 커널 빌드 형식) 두 라벨셋을 모두 인식하고 버전 값 길이 제한도 완화해
받아들인다.
...
`LAP-SCR-013`/`SYS-003` 기기 정보 화면의 네 필드는 사진이나 자동 검사 증거가 아직 없어도 입력칸을 먼저
표시하고 `listing_checklist_item`의 수동 기기 정보 컬럼에 저장한다. 이 무증빙 수동 저장은 다른 OCR 사진
항목에는 적용하지 않는다.
