...
검증된 서버 기본 정책이 항상 먼저 적용된다. AI는 기본 항목을 삭제하거나 필수 여부를 바꾸지 못한다.

기본 항목은 기기 식별, 외관, 디스플레이, 전원·부팅, 키보드, 터치패드, 충전, 실제 사양 확인, 계정 로그아웃·초기화로 구성한다.

- Windows: `battery-report.html`과 `DxDiag.txt` 등록 항목을 추가한다. 파일 생성이 불가능한 경우 화면 캡처를 안내한다.
...
- 승인 목록 밖의 기능은 `reviewCandidates`로만 반환하고 필수 항목으로 만들지 않는다.
...
노트북 등록 요청에 `confirmedFeatures`를 최대 5개까지 전달할 수 있다. 서버는 등록 시점에 별도의 DRAFT 체크리스트 템플릿과 매물 체크리스트 항목을 생성한다. 따라서 이후 공통 정책이 변경되어도 이미 등록한 매물의 항목은 바뀌지 않는다.

노트북 외 기존 기기는 종전처럼 최신 PUBLISHED 템플릿을 스냅샷으로 사용한다.
